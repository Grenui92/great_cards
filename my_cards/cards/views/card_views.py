...
class CardsListView(ListView):
    ...
    def get_queryset(self):
        """
        The get_queryset function is used to filter the queryset of cards that
        are returned by the view.
        In this case, we want to return all cards in a collection.
        The collection_id is passed as an argument to the view and then used
        here to get all cards in that particular collection.
    ...
    def get_context_data(self, *_, object_list=None, **kwargs):
        """
        The get_context_data function is a function that allows you to pass
        additional context variables to the template.
        The get_context_data function takes in an optional object_list
        parameter, which is used by ListViews and DetailViews.
        It also takes in any number of keyword arguments (kwargs).
        The **kwargs are passed into the superclass's get_context_data method,
        which returns a dictionary containing all of the context variables for
        this view.
...
class CreateCardView(View, MessageMixin):
    ...
    def get(self, request):
        ...
        It takes in the request and returns a rendered template with either an
        error message or success message.
        If the form is valid, it saves the form but does not commit it yet,
        then calls get_new_card from CardsServices which creates a new card
        object and adds it to the database.
        The function then renders either an error or success message depending
        on whether there was an issue creating the card.
...
class CardDeleteView(View):
    ...
    def post(self, request, card_id, collection_id):
        ...
        The function then gets the collection by its id and gets the card from
        that specific collection.
        Then it removes that specific card from that specific collections
        cards list using .remove().
